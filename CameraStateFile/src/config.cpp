...
namespace CameraStateFile {
  ...
  json Config::evaluateConfig(string pointerToEval) {
    ...
    vector<json::json_pointer> json_to_eval = CameraStateFile::findKeyInJson(eval_json, "kernels", true);
  ...
  vector<string> Config::findKey(string key, bool recursive) {
    ...
    vector<json::json_pointer> ptrs = CameraStateFile::findKeyInJson(subConf, key, recursive);
    for(auto &e : ptrs) {
