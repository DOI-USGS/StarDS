...
namespace CameraStateFile {
  ...
  string getConfigDirectory() {
    ...
    fs::path debugDbPath = fs::absolute(_SOURCE_PREFIX) / "CameraStateFile" / "db";
    fs::path installDbPath = fs::absolute(condaPrefix) / "etc" / "CameraStateFile" / "db";
    ...
    SPDLOG_TRACE("CameraStateFile DB Path: {}", dbPath.string()); 
  ...
  string getRestUrl() {
    ...
    SPDLOG_TRACE("CameraStateFile REST URL: {}", spiceqlRestUrl); 
