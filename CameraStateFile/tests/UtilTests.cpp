...
TEST(UtilTests, checkNaifErrors) {
  ...
  try { 
    ...
    EXPECT_PRED_FORMAT2(CameraStateFile::AssertExceptionMessage, e, "The attempt to load \"does/not/exist\" by the routine FURNSH failed.");
  }
}
...
TEST(UtilTests, testGetPathsFromRegex) {
  ...
  fs::path temp = fs::temp_directory_path() / CameraStateFile::gen_random(10); 
...
TEST(UtilTests, testJson2DArrayTo2DVector) { 
  ...
  try { 
    ...
    EXPECT_PRED_FORMAT2(CameraStateFile::AssertExceptionMessage, e, "Input json is not a valid 2D Json array:");
  } 
}
