...
TEST_F(TestConfig, FunctionalTestConfigEval) {
  ...
  EXPECT_EQ(CameraStateFile::getKernelsAsVector(config_eval_res["clementine1"][pointer]).size(), expected_number);
  EXPECT_EQ(CameraStateFile::getKernelsAsVector(pointer_eval_res[pointer]).size(), expected_number);
  ...
  expected_number = 1;
  EXPECT_EQ(CameraStateFile::getKernelsAsVector(config_eval_res[pointer]).size(), expected_number);
...
TEST_F(TestConfig, FunctionalTestConfigAccessors) {
  ...
  MockRepository mocks; 
  mocks.OnCallFunc(CameraStateFile::ls).Return(paths);
...
TEST_F(TestConfig, FunctionalTestsConfigKeySearch) {
  ...
  MockRepository mocks;
  mocks.OnCallFunc(CameraStateFile::findKeyInJson).Return(pointers);
...
TEST_F(TestConfig, FunctionalTestsConfigGetParentPointer) {
  MockRepository mocks;
  mocks.OnCallFunc(CameraStateFile::getRootDependency).Return("");
