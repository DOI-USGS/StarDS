 ...
 */
class MetadataBlockTest : public ::testing::Test {
protected:
    std::vector<std::string> temp_files;  // Track all temp files created

    void SetUp() override {
        // Nothing to do in setup - files created on demand
    }
    ...
    /**
     * @brief Creates a unique temporary filename for testing
     * @param prefix Optional prefix for the filename
     * @return Unique temporary filename that will be auto-cleaned
     */
...
TEST_F(MetadataBlockTest, VersionCheck) {
    ...
    // Create a valid file with current format version (2)
    {
        auto store_write = StarDataset::create(testFile);
        store_write->meta.put("test", NDArray<int64_t>({}, 123));
        store_write->flush();
