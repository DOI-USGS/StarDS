...
def test_create_on_non_existing_file(tmp_path):
    ...
    filepath = str(tmp_path / "new_file.stards")
...
def test_create_on_existing_file_overwrites(tmp_path):
    ...
    filepath = str(tmp_path / "existing.stards")
    ...
    assert os.path.exists(filepath)
    store_check = pystards.StarDataset.open(filepath, mode="r")
    assert "old_data" in store_check
    store_check.close()

    # Create again - should overwrite
    store2 = pystards.StarDataset.create(filepath)
...
def test_open_rw_on_non_existing_file_creates(tmp_path):
    ...
    filepath = str(tmp_path / "new_via_open.stards")

    # File doesn't exist
    assert not os.path.exists(filepath)

    # Open with rw mode should create it
    store = pystards.StarDataset.open(filepath, mode="rw")
    store["data"] = np.array([1, 2, 3])
    store.flush()
    store.close()
...
def test_open_readonly_on_non_existing_file_fails(tmp_path):
    ...
    filepath = str(tmp_path / "missing.stards")
...
def test_open_on_corrupt_file_fails(tmp_path):
    ...
    filepath = str(tmp_path / "corrupt.stards")
...
def test_open_rw_vs_create_behavior(tmp_path):
    ...
    filepath1 = str(tmp_path / "via_open_rw.stards")
    filepath2 = str(tmp_path / "via_create.stards")
    ...
    store1 = pystards.StarDataset.open(filepath1, mode="rw")
    store1["data"] = np.array([1, 2, 3])
    store1.flush()
    store1.close()

    store2 = pystards.StarDataset.create(filepath2)
    ...
    store1 = pystards.StarDataset.open(filepath1, mode="r")
    store2 = pystards.StarDataset.open(filepath2, mode="r")
