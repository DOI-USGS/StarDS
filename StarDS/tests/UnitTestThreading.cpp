...
// Each test runs in its own temp directory (auto-deleted); testFile lives inside
// it, unique per test (parallel-safe).
class ThreadingTest : public star_test::TempDirTest {
protected:
    std::string testFile;

    void SetUp() override {
        star_test::TempDirTest::SetUp();
        testFile = tempFilePath("test_threading.stards");
    }
...
TEST_F(ThreadingTest, DatasetThreadingInitialization) {
    ...
    setNumThreads(4);
    std::string testFile2 = tempFilePath("test_threading2.stards");
...
TEST_F(ThreadingTest, CompressionRatiosUnchanged) {
    ...
    // Single-threaded
    setNumThreads(1);
    std::string file1 = tempFilePath("test_threading_st.stards");
    {
    ...
    // Multi-threaded
    setNumThreads(4);
    std::string file2 = tempFilePath("test_threading_mt.stards");
    {
